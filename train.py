...
from datasets import Knee25DDataset, TriPlaneDataset, PseudoLabelLoader
from models import EfficientNetV2S25D, TriPlaneModel
...
def _main_triplane(
    ...
    # ── 1. 加载标签 ───────────────────────────────────────────────
    if pseudo_csv and Path(pseudo_csv).exists() and gold_csv:
        logger.info("=" * 60)
        ...
        logger.info("=" * 60)
        logger.info("Tri-Plane 训练模式 — Gold-Only")
        logger.info("=" * 60)
        ...
        # Simple split: 80/20 by study
        study_ids = sorted(set(
            series_df[series_df["Anatomical_Plane"] == "Sagittal"]["StudyInstanceUID"]
        ) & set(labels_df["StudyInstanceUID"]))
        ...
        train_labels_df = labels_df[labels_df["StudyInstanceUID"].isin(train_ids)].set_index("StudyInstanceUID")
        val_labels_df = labels_df[labels_df["StudyInstanceUID"].isin(valid_ids)].set_index("StudyInstanceUID")
...
if __name__ == "__main__":
    ...
    is_triplane = (
        args.triplane
        or config_check.get("model", {}).get("arch", "").startswith("triplane")
        or config_check.get("experiment", {}).get("stage") == "phase2"
    )

    if is_triplane:
        # Tri-plane mode — load metadata and route to _main_triplane
        series_df = pd.read_csv(config_check["paths"]["series_csv"])
        pseudo_csv = args.pseudo_csv or config_check.get("pseudo_label", {}).get("pseudo_csv")
        gold_csv = config_check["paths"]["train_csv"]
