...
CFG = {
    ...
    # ★ spec 词表标签数据集: 把 report_labels_v2.csv 上传到 v5 标签数据集的新版本
    #   (本地 scripts/report_extractor_v2.py 生成, 词表规则提取器, 无训练无泄漏)
    ...
    'seed': 442,                   # ★ spec 专家成员: 换标签源 (词表) 的新集成成员, seed 442 保持头初始化多样
    'lr': 2e-4,
    'backbone_lr': 1e-5,
    'weight_decay': 1e-4,
    'lr_t0': 15,
...
# ---- 全局随机种子 — spec 专家成员 (词表标签监督) ----
#   与 v5 s1/s2/s3 的差异: 标签源 = 词表提取器 (report_labels_v2.csv), 其余数据侧全同
import random
...
SEED_TAG = 'spec'                         # 产物文件名后缀 (best_model_spec.pt)
